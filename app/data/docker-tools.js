  ...
  dockerJS.ps((err, services) => {
    if (err) callback(err);
      ...
      const names = services
        .map((s) => s.Names[0].slice(1))
        .filter((ss) => ss.startsWith(TCPDUMP_SERVICE_NAME));
      callback(null, names);
    }
  });
}
...
function runTcpdump(
  ...
  const machines = Object.keys(
    _.omit(machinesToBeSniffed, (v, k, object) => v == false)
  );
  async.each(
    machines,
    (m, c) => {
  ...
  async.waterfall(
    [
      (cb) => {
        ...
        dockerJS.exec(
          nameService,
          `ip route replace default via ${ipRouter}`,
          cb
        );
      },
    ],
...
function get(name) {
  ...
  if (!ret) throw new Error("Not existent hack tool");
  if (!ret.default_options) ret.default_options = {};
  return ret;
}

  ...
  async.waterfall(
    [
      ...
      (cb) =>
        Checker.checkParams(
          req.body,
          ["networkList", "clistToDraw", "clistNotToDraw", "canvasJSON"],
          cb
        ),
      // Creation of directories if volume is setted
      (cb) =>
        dockerVolumes.destroyOldVolumes(
          req.params.namelab,
          req.body.clistToDraw,
          req.body.clistNotToDraw,
          cb
        ),
      ...
      (cb) =>
        dockerVolumes.createVolumeDirs(
          req.params.namelab,
          req.body.clistToDraw,
          cb
        ),
      ...
      (cb) => {
        ...
        try {
          jsonCompose = dockerConverter.JSONDockerComposeConvert(
            req.body.clistToDraw,
            req.body.networkList
          );
...
function getContainersInfos(req, res) {
  log.info("[Hakiba TEST] Getting containers infos");
  async.waterfall(
    [
      (cb) => Checker.checkParams(req.params, ["namelab"], cb),
      (cb) => dockerServices.getNetworksLab(req.params.namelab, cb),
    ...
    (err, data) => {
      if (data === null) {
        log.info("No containers launched");
      }
      AppUtils.response("NETWORK LIST", res, err, data, true);
    }
  );
  ...
  async.waterfall(
    [
      ...
      (cb) =>
        Checker.checkParams(
          req.body,
          ["name", "selectedImage", "isInteractive", "isDaemonized"],
          cb
        ),
      (cb) => {
  ...
  async.waterfall(
    [
      (cb) =>
        Checker.checkParams(
          req.body,
          ["namelab", "namerepo", "dockername", "pathContainer"],
          cb
        ),
      ...
      (dockerInfos, cb) => {
        const pathLab = path.join(
          dockerInfos.mainPath,
          dockerInfos.nameRepo,
          dockerInfos.labName
        );
        destinationDir = path.join(
          "public",
          "downloads",
          `${dockerInfos.nameRepo}_${dockerInfos.labName}`
        );
        destinationPath = path.join(
          destinationDir,
          path.basename(dockerInfos.pathContainer)
        );
        // Fix for space directories
        destinationPath = `"${destinationPath}"`;
        dockerComposer.cpFrom(
          pathLab,
          dockerInfos.dockerName,
          dockerInfos.pathContainer,
          destinationPath,
          cb
        );
...
function dirExists(req, res) {
  async.waterfall(
    [
      ...
      (cb) => {
        pathExists(req.body.filename).then((exists) => {
          cb(null, exists);
        });
      },
    ],
...
function attachNetwork(req, res) {
  ...
  async.waterfall(
    [
      ...
      (cb) =>
        dockerServices.attachServiceToNetwork(
...
function detachNetwork(req, res) {
  ...
  async.waterfall(
    [
      (cb) =>
        Checker.checkParams(req.query, ["networkname", "servicename"], cb),
      (cb) =>
        dockerServices.detachServiceToNetwork(
          req.query.servicename,
          req.query.networkname,
          cb
        ),
    ],
    (err, data) => {
...
exports.getContainersInfos = getContainersInfos;
